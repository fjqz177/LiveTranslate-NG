...
from livetranslate.core.translator import make_openai_client

BENCH_SENTENCES = {
    "ja": [
        "こんにちは、今日はいい天気ですね。",
        "この映画はとても面白かったです。",
        "明日の会議は何時からですか？",  # noqa: RUF001
        "日本の桜は本当に美しいですね。",
        "新しいレストランに行ってみましょう。",
    ],
    "en": [
    ...
    "zh": [
        "你好，今天天气真不错。",  # noqa: RUF001
        "那部电影真的很有意思。",
        "明天的会议几点开始？",  # noqa: RUF001
        "日本的樱花真的很美丽。",
        "我们去试试那家新餐厅吧。",
    ],
    "ko": [
...
def run_benchmark(
    ...
    def _test_model(m: dict[str, Any]) -> dict[str, Any]:
        name = m["name"]
        ...
        try:
            ...
            for i, text in enumerate(sentences):
                ...
                try:
                    ...
                    # The openai SDK stubs cannot express our dynamic provider
                    # payloads; the call result is treated as a typed-blob.
                    stream = cast(
                        "Any",
                ...
                lines.append(
                    f"  Round {i + 1}: {total_ms:7.0f}ms (TTFT {ttft:6.0f}ms) | {result_text[:60]}"
                )
