...
class ASREngine(ASREngineBase):
    ...
    def _read_pad_seconds(value=None) -> float:
        ...
        try:
            ...
            log.warning(
                f"Invalid Whisper pad seconds={value!r}; using default {DEFAULT_PAD_SECONDS:g}s"
            )
            return DEFAULT_PAD_SECONDS
    ...
    def transcribe(
        ...
        for seg in segments:
            ...
            if word_timestamps and seg.words:
                words.extend(WordTiming(word=w.word, start=w.start, end=w.end) for w in seg.words)
