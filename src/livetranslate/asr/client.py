...
class ASRClient:
    ...
    def status(self) -> str:
        if (
            ...
            and self._process.exitcode is not None
            and self._status not in ("failed", "stopping", "stopped")
        ):
            self._status = "exited"
        return self._status
    ...
    def wait_ready(self, timeout: float | None = None):
        ...
        # Release the lock during the (long) startup wait so a concurrent
        # shutdown() is not blocked for up to ready_timeout.
        ...
        with self._lock:
            ...
            self._status = "ready"
        log.info(f"ASR worker ready: pid={self.pid}, {response.get('payload') or {}}")
    ...
    def ping(self):
        """Cheap liveness probe so idle worker death is detected early."""
        self._request("ping", {}, timeout=min(2.0, self.request_timeout))
    ...
    def shutdown(self):
        with self._lock:
            ...
            conn = self._conn
        # Do not read from the pipe here: an in-flight transcribe owns the
        # response stream, and this shutdown request is queued behind it.
        if process.is_alive() and conn is not None:
            with contextlib.suppress(BrokenPipeError, EOFError, OSError):
                conn.send({"id": uuid.uuid4().hex, "type": "shutdown", "payload": {}})
        process.join(timeout=self.shutdown_timeout)
        if process.is_alive():
            log.warning(f"ASR worker did not exit, terminating pid={process.pid}")
            process.terminate()
            process.join(timeout=self.shutdown_timeout)
            # Leave handles attached so an in-flight request can observe the
            # exit code and fail fast instead of spinning to its deadline.
            return
        self._close_handles()
    ...
    def _request(self, request_type: str, payload: dict, timeout: float):
        ...
        # The blocking cross-process wait runs without the lock so a
        # concurrent shutdown()/terminate() never stalls behind a hung worker.
        try:
    ...
    def _recv_response(self, timeout: float, expected_id: str | None):
        # Capture refs once: shutdown()/terminate() may detach them midway.
        conn = self._conn
        process = self._process
        ...
        while True:
            ...
            if remaining <= 0:
                self._status = "failed"
                self.terminate()
                raise ASRWorkerTimeout(f"ASR worker response timed out after {timeout:g}s")
            ...
            if process is not None and process.exitcode is not None:
                self._status = "exited"
                raise ASRWorkerExited(f"ASR worker exited with code {process.exitcode}")
